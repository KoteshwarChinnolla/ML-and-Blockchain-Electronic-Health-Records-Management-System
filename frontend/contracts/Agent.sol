...
pragma solidity ^0.8.0; // UPDATED: Modern Pragma
...
contract Agent {
    ...
    function add_agent(string memory _name, uint _age, uint _designation, string memory _hash) public returns(string memory){
        ...
        if(_designation == 0){
            ...
            patientList.push(addr); // UPDATED: Removed '-1'
            return _name;
        }
            ...
            d.record = _hash;
            doctorInfo[addr] = d;
            doctorList.push(addr); // UPDATED: Removed '-1'
            return _name;
        }
        else{
            revert("Invalid Designation"); // UPDATED: Added error message
        }
    }
    ...
    function get_doctor(address addr) view public returns (string memory , uint, string memory){
        return (doctorInfo[addr].name, doctorInfo[addr].age, doctorInfo[addr].record);
    }
    ...
    function permit_access(address addr) payable public {
        ...
        doctorInfo[addr].patientAccessList.push(msg.sender); // UPDATED
        patientInfo[msg.sender].doctorAccessList.push(addr); // UPDATED
    }
    ...
    function insurance_claimm(address paddr, uint _diagnosis, string memory  _hash) public {
        ...
        for(uint i = 0; i < doctorInfo[msg.sender].patientAccessList.length; i++){
            if(doctorInfo[msg.sender].patientAccessList[i] == paddr){
                payable(msg.sender).transfer(2 ether); // UPDATED: Explicit payable cast
                creditPool -= 2;
                patientFound = true;
            }
        }
    ...
    function remove_element_in_array(address[] storage Array, address addr) internal returns(uint)
    {
        ...
        if(!check){
            ...
            if(Array.length == 1){
                Array.pop(); // UPDATED: Use pop() instead of delete/length--
            }
            else {
                Array[del_index] = Array[Array.length - 1];
                Array.pop(); // UPDATED: Swap and pop
            }
            // Array.length--; // REMOVED: Cannot manually change length in 0.8.x
            return del_index;
        }
    }
